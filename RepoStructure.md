...
> **CodeSnip _pagoda_ has been abandoned**

These notes apply to the _pagoda_ branch of the CodeSnip repo and explain the design of the directory structure.

> **The _pagoda_ repo is quite messy - that's because I was slowly changing it over to a new structure, and for reasons of not wanting to clobber the merging of changes from the repo _master_, I couldn't do it all in one go.**

This document presents the new structure I was aiming at, lists the original structure, still used in _master_, and shows both how I was proposing to change over without breaking too much at once and how far I'd got before _pagoda_ was abandoned (see: _Process for Converting to New Directory Structure_ below).

New Directory Structure
-----------------------

The code committed to the repo would ultimately have had this structure:
...
In addition to _Docs_ and _Src_ top level directories there was also to be a _Build_ directory to store files output by the build process. This directory was not to be included in the repo. The structure of _Build_ was to be:
...
+ **Src**
    ...
    The only files stored directly in _Src_ should be makefiles, project group files and license files. The remaining source is grouped into sub-projects, each stored in its own sub-folder of _Src_. These sub-projects are:
    ...
    + **Src\Main**
        ...
        + **Src\Main\AutoGen**
            ...
            Only the read-me and license files are recorded in the repo. Generated files are not since they can be regenerated from the makefile at will.
...
Make files, _.cfg_ and Delphi project files should be updated over time to store binary files into the new _Build_ directory. This process can proceed at any time since the affected folders are not part of the repo and no consideration of merges has to take place.
...
Migration from the old structure to the new is complicated by the need to merge changes from _master_ into the _pagoda_ code tree.

This means that, while the need to perform such merges continues, it is difficult to make all the changes. Only when the divergence between _pagoda_ and  _master_ becomes so great that future merges are too problematic can the final transition be made.
...
Of these directories _Test\Bin_ and _Test\Exe_ were excluded from the repo.
...
+ **Src**
    ...
    Since _Src_ is affected by merges from _master_ a staged approach is needed:

    - **_DONE:_** Create new _Main_ sub-directory.

    - **_DONE:_** Move existing files created in _pagoda_ since it was branched from _master_ into _Src\Main_, updating project files as required.

    - **_IN PROGRESS_** Add any new CodeSnip project files directly into _Src\Main_ instead of _Src_.

    - When merging from _master_ has ceased do the following:
...
+ **Src\3rdParty**
    ...
    **_Warning re merge conflicts:_** _Src\3rdParty_ was branched from _master_ so any merges involving changes to the directory will cause a tree conflict, because _Src\3rdParty_ has been deleted. Such a conflict should be resolved by rejecting the change. If any of the changes to _master\Src\3rdParty_ are desirable then merge _master\Src\3rdParty_ into _pagoda\Src\Main\Imports_.
...
+ **Src\AutoGen**
    ...
    **_Warning re merge conflicts:_** _Src\AutoGen_ was branched from _master_ so any merges involving changes to the directory will cause a tree conflict, because _Src\AutoGen_ has been deleted. Such a conflict should be resolved by rejecting the change. If any of the changes to _master\Src\AutoGen_ are desirable then merge _master\Src\AutoGen_ into _pagoda\Src\Main\AutoGen_.

+ **Src\Help and sub-directories**

    **_These directories do not need to be moved or changed._**

    They are affected by merges from _master_, so **do not rename** any directories until merges have ceased.
...
+ **Src\Lib and sub-directories**
    ...
    **_All changes now complete_**
    ...
    This was safe because _Src\Lib_ was added to the _pagoda_ branch after it branched from _master_ so the directory isn't affected by merges from _master_.

+ **Src\Portable**

    **_This directory does not need to be moved or changed._**

    _Src\Portable_ was added to _pagoda_ after it was branched from _master_ so the directory is not affected by any merges from _master_.
...
+ **Src\Res and its sub-directories**
    ...
    Since _Src\Res_ is affected by merges from _master_ a staged approach is needed:
    ...
    - When merging from _master_ has ceased proceed as follows as follows:
...
+ **Test** and its sub-directories.
    ...
    **_Warning re merge conflicts:_** The _Tests_ directory, and its only sub-directory, _Tests\Src\DUnit_ was branched from _master_. Therefore so any merges involving changes within the directory will cause a tree conflict because _Tests_ has been deleted. Such a conflict should be resolved by rejecting the change. If any changes to _master\Tests\Src\DUnit_ are desirable then merge _master\Tests\Src\DUnit_ into _pagoda\Src\Tests\DUnit_.
    If any other  directories have been added to _master\Tests_ or
    _master\Tests\Src_ then a decision needs to be made as to if or how to reflect those changes in _pagoda\Src\Tests_
