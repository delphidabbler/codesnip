...
These notes apply to the _pavilion_ branch of the CodeSnip Git repo and explain the design of the directory structure.

> **At present the _pavilion_ repo is quite messy - that's because I'm slowly changing it over to a new structure, and for reasons of not wanting to clobber the merging of changes from the repo _master_, I can't do it all in one go.**

This document presents the new structure I'm aiming at, lists the original structure, still used in _master_, and shows both how I'm proposing to change over without breaking too much at once and how far I've got (see: _Process for Converting to New Directory Structure_ below).

The new structure should be in place in the _pavilion_ branch before the end of the early development of CodeSnip 5.

New Directory Structure
-----------------------

The code committed to the Git repo will ultimately have this structure:
...
In addition to _Docs_ and _Src_ top level directories there will also be a _Build_ directory that stores files output by the build process. This directory will not be included in Git. The structure of _Build_ is:
...
+ **Src**
    ...
    + **Src\Main**
        ...
        + **Src\Main\AutoGen**
            ...
            Only the read-me and license files are recorded in Git. Generated files are not since they can be regenerated from the makefile at will.
...
Make files, _.cfg_ and Delphi project files will be updated over time to store binary files into the new _Build_ directory. This process can proceed at any time since the affected folders are not part of Git and non consideration of merges has to take place.
...
Migration from the old structure to the new is complicated by the need to merge changes from Git _master_ into the _pavilion_ code tree.

This means that, while the need to perform such merges continues, it is difficult to make all the changes. Only when the divergence between _pavilion_ and  _master_ becomes so great that future merges are too problematic can thefinal transition be made.

The structure of _Src_ in _pavilion_ before the restructuring began was as follows:
...
Of these directories _Test\Bin_ and _Test\Exe_ were excluded from Git.
...
+ **Src**
    ...
    - **_DONE:_** Move existing files created in _pavilion_ since it was branched from _master_ into _Src\Main_, updating project files as required.
...
+ **Src\Lib and sub-directories**
    ...
    **_All changes now complete_**
    ...
    This was safe because _Src\Lib_ was added to the _pavilion_ branch after it branched from _master_ so the directory isn't affected by merges from _master_.

+ **Src\Portable**

    **_This directory does not need to be moved or changed._**

    _Src\Portable_ was added to _pavilion_ after it was branched from _master_ so the directory is not affected by any merges from _master_.
