...
var
  ...
  SaveStm: TStream;     // temp stream used to generate endcoded data
resourcestring
...
begin
  ...
  try
    ...
    { todo: replace with call to new method of TXMLDocHelper that takes encoding
            type and sets XML doc encoding and creates encoded data }
    fXMLDoc.Encoding := 'UTF-8';
    SaveStm := TMemoryStream.Create;
    try
      fXMLDoc.SaveToStream(SaveStm);
      Result := TEncodedData.Create(SaveStm, etUTF8, True);
    finally
      SaveStm.Free;
    end;
